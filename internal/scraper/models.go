		...
		Hits struct {
			...
			Hits     []struct {
				...
				Source0 struct {
					...
					ID                    int      `json:"id"`
					AvailableMediaTypes   []string `json:"available_media_types"`
					OpenHouseDatetimeSlot []struct {
						OpenHouseID        string `json:"open_house_id"`
						OpenHouseDateRange struct {
							Gte time.Time `json:"gte"`
							Lte time.Time `json:"lte"`
						} `json:"open_house_date_range"`
						OpenHouseDate     time.Time `json:"open_house_date"`
						IsNvmOpenHouseDay string    `json:"is_nvm_open_house_day"`
					} `json:"open_house_datetime_slot"`
					Surrounding              []string `json:"surrounding"`
					ObjectTypeSpecifications struct {
						Appartment struct {
							Type string `json:"type"`
